...
public:
	...
	ModeBase* Get(int uid);// uidで取得
	ModeBase* Get(const char* name);// 名前で取得
	int GetId(ModeBase* mode);// モードからuidを取得
	int GetId(const char* name);// 名前からuidを取得
	const char* GetName(ModeBase* mode);// モードから名前を取得
	const char* GetName(int uid);// uidから名前を取得
	void Clear();
